...
public class Checkerboard {
    ...
    Map<Integer, Square> getSquareMap() {
        return squareMap;
    }
        ...
        squareMap = getPlayableSquaresList().stream()
                .collect(Collectors.toMap(Function.identity(), (Integer v) -> {
                    if (darkPieces.contains(v)) {
                        return new Square(v, PieceType.DARK, AdjacentSquaresLocator.getAdjacentSquaresNumbers(v));
                    } else if (lightPieces.contains(v)) {
                        return new Square(v, PieceType.LIGHT, AdjacentSquaresLocator.getAdjacentSquaresNumbers(v));
                    } else {
                        return new Square(v, PieceType.EMPTY, AdjacentSquaresLocator.getAdjacentSquaresNumbers(v));
                    }
                }));
    }
